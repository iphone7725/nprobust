	...
	***********************************************************************
	...
	if  ("`bwselect'"=="imse-dpi" |  "`bwselect'"=="all") {
		...
		for (e=1; e<=neval_imse; e++) {
			...
			bw_mse = nprobust_lp_mse_dpi(Y, X, C, eval_imse[e], `p', `q', `deriv', `even', "`kernel'", c_bw, `bwcheck', `bwregul', "`vce_select'", `nnmatch', dups, dupsid, "`interior'") 
			V_h = bw_mse[3]  
			B_h = bw_mse[4]  
			V_b = bw_mse[5]  
			B_b = bw_mse[6]  
			bws_C_dpi[e,.] = V_h , B_h, V_b, B_b
		}
	...
	if  ("`bwselect'"=="imse-rot" |  "`bwselect'"=="all") {
		...
		for (e=1; e<=neval_imse; e++) {
			...
			bws_C_rot[e,.]= s2_p/f0_pilot, mp1^2, s2_q/f0_pilot, mq1^2
      }
    ...
    *if (!is.null(rho))  b_mse_dpi = h_mse_dpi/rho
    Result[e,2::3] = h_mse_dpi,  b_mse_dpi	  
      ...
      if  ("`bwselect'"=="mse-rot" | "`bwselect'"=="all") {
        ...
        b_mse_rot = `C_c'*((s2_q/f0_pilot)/(N*mq1^2))^(1/(2*`q'+3))
