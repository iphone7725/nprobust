...
program define lprobust, eclass
	syntax anything [if] [in] [, eval(varname) neval(real 0) deriv(real 0) p(real 1) h(string) b(string) rho(real 1) kernel(string) bwselect(string) bwcheck(real 0) vce(string) level(real 95) separator(integer 5) bwregul(real 1) interior genvars plot graph_options(string)]
	...
	**** error check: bw()
	if ("`h'" == "") {
		tempvar temp_h
		tempvar temp_b
		qui lpbwselect `y' `x' if `touse', eval(`temp_grid') deriv(`deriv') p(`p') kernel(`kernel') vce(`vce') bwselect(`bwselect') bwregul(`bwregul') bwcheck(`bwcheck') `interior'  genvars
