...
{browse "https://sites.google.com/site/nppackages/nprobust/Calonico-Cattaneo-Farrell_2018_JASA.pdf":Calonico, Cattaneo and Farrell (2018a)}. It also implements other 
estimation and inference procedures available in the literature. See Wand and Jones (1995) and Fan and Gijbels (1996) for background references.{p_end}
...
{p 4 8}{opt kernel}({it:kernelfn}) specifies the kernel function used to construct the local-polynomial estimator(s). Options are: {opt tri:angular}, {opt epa:nechnikov}, and {opt uni:form}.
...
{p 4 8}{opt interior} optional option to set all evaluation points to be interior points. This option affects only data-driven bandwith selection via {help lpbwselect:lpbwselect}.{p_end}
